...
class ManifestTest(unittest.TestCase):
    ...
    def test_apply_recommended_updates_executed_command_in_manifest(self) -> None:
        repo_root = Path(__file__).resolve().parents[1]
        with tempfile.TemporaryDirectory() as tmp:
            temp_root = Path(tmp)
            config_path = temp_root / "config.json"
            output_dir = temp_root / "output"
            cache_dir = output_dir / "cache"
            recommendation_path = temp_root / "recommended.json"
            config_path.write_text(
                json.dumps(
                    {
                        "paths": {
            ...
            recommendation_path.write_text(
                json.dumps({"strategy_overrides": {"holding_count": 2}}, indent=2),
                encoding="utf-8",
            )
        ...
        self.assertEqual(payload["requested_command"], "apply-recommended")
        self.assertEqual(payload["executed_command"], "signals")
        self.assertEqual(
            payload["extra"]["recommended_config_path"],
            str(recommendation_path),
        )
        self.assertIn(str(output_dir / "applied_recommended_config.json"), payload["outputs"])
        self.assertIn(str(output_dir / "rebalance_signals.csv"), payload["outputs"])
